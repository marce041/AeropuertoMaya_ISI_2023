<?php
    ...
    class elementosMenu
    {
        public function mostarTablaClase()
        {
         ...
         while($i<$rows2){
             if($nombre_tabla[$i]=='ModificarClase'){
                 $j=1;
             }
             $i+=1;
         }
         ...
         if($j == 1){
            ...
            while ($data = mysqli_fetch_assoc($query))
            {
                ...
                "
                    <tr>
                        ...
                        <form action='formEditNave.php?id=$data[Id_Clase]' method='POST' name='form2'>
                        <th scope='row'>$data[Id_Clase]</th>
                        <td>$data[Tipo_Clase]</td>
                        <td>$data[Descripcion]</td>
                        <td>$data[MultiplicadorPrecio]</td>
            ...
            "
                ...
                <table>
                <thead>
                <th scope='col'><a class='btn btn-danger' href='../Procesos/Reportes/clasepdf.php'><i class='fa-solid fa-file-pdf'></i> Generar PDF
                </a></th>
                            <th scope='col'>
                            <a class='btn btn-success' href='../Procesos/Reportes/clasexlsx.php'><i class='fa-regular fa-file-excel'></i> Generar EXCEL
                            </a></th>
                            </thead>
                            </table>
