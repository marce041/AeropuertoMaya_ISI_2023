<?php
    ...
    class elementosMenu
    {
        public function mostarTablaRol()
        {
         ...
         while($i<$rows2){
             if($nombre_tabla[$i]=='ModificarRol'){
                 $j=1;
             }
             $i+=1;
         }
         ...
         if($j == 1){
            ...
            while ($data = mysqli_fetch_assoc($query))
            {
                ...
                "
                    <tr>
                        ...
                        <form action='formEditNave.php?id=$data[Id_Rol]' method='POST' name='form2'>
                        <th scope='row'>$data[Id_Rol]</th>
                        <td>$data[Nombre]</td>
                        <td>$data[Descripcion]</td>
                        <td>$data[Activo]</td>
            ...
            "
                ...
                <table>
                <thead>
                <th scope='col'><a class='btn btn-danger' href='../Procesos/Reportes/rolpdf.php'><i class='fa-solid fa-file-pdf'></i> Generar PDF
                </a></th>
                            <th scope='col'>
                            <a class='btn btn-success' href='../Procesos/Reportes/rolxlsx.php'><i class='fa-regular fa-file-excel'></i> Generar EXCEL
                            </a></th>
                            </thead>
                            </table>
