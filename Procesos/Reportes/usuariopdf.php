...
try {
   ...
   $fecha=explode("/", $datos2);
    ...
    $path = "UsuarioPdfSelectUser-".$fecha[2]."-".$fecha[1]."-".$fecha[0]."_".$hora[0]."_".$hora[1]."_".$hora[2].".log";
    error_log("\n" .date("d/m/Y H:i:s")." ". $e->getMessage(),3,$path);
    header("Location: ../../Consultas/Consultausuarios.php");
}
...
function Header()
{
    ...
    $this->cell(52,10,'Nombre de Usuario',1,0,'C',0);
    $this->cell(60,10,utf8_decode('Contraseña Encriptada'),1,0,'C',0);
...
try {
   ...
   $fecha=explode("/", $datos2);
    ...
    $path = "UsuarioPdfSelectUsuario-".$fecha[2]."-".$fecha[1]."-".$fecha[0]."_".$hora[0]."_".$hora[1]."_".$hora[2].".log";
    error_log("\n" .date("d/m/Y H:i:s")." ". $e->getMessage(),3,$path);
    header("Location: ../../Consultas/Consultausuarios.php");
}
...
while($row=$resultado->fetch_assoc()){
    ...
    $pdf->cell(52,10,$row['Usuario'],1,0,'C',0); 
    $pdf->SetFont('Arial','',7);
    $pdf->cell(60,10,$row['Pass'],1,0,'C',0);
    $pdf->SetFont('Arial','',14);
    if($row['Estado']==1){
        $pdf->cell(40,10,'Habilitado',1,0,'C',0);
    }else if($row['Estado']==0){
        $pdf->cell(40,10,'Deshabilitado',1,0,'C',0);
