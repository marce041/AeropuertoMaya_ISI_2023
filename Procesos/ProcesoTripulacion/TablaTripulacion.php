<?php
    ...
    class elementosMenu
    {
        public function mostarTablaTripulacion()
        {
         ...
         while($i<$rows2){
             if($nombre_tabla[$i]=='ModificarTripulacion'){
                 $j=1;
             }
             $i+=1;
         }
         ...
         if($j == 1){
            ...
            while ($data = mysqli_fetch_assoc($query))
            {
                ...
                "
                    <tr>
                        ...
                        <form action='formEditNave.php?id=$data[Id_Tripulacion]' method='POST' name='form2'>
                        <th scope='row'>$data[Id_Tripulacion]</th>
                        <td>$data[Cargo]</td>
                        <td>$data[Horas_Vuelo]</td>
                        <td>$data[Tipo_Licencia]</td>
            ...
            "
                ...
                <table>
                <thead>
                <th scope='col'><a class='btn btn-danger' href='../Procesos/Reportes/tripulacionpdf.php'><i class='fa-solid fa-file-pdf'></i> Generar PDF
                </a></th>
                            <th scope='col'>
                            <a class='btn btn-success' href='../Procesos/Reportes/tripulacionxlsx.php'><i class='fa-regular fa-file-excel'></i> Generar EXCEL
                            </a></th>
                            </thead>
                            </table>
