<?php
    ...
    class elementosMenu
    {
        public function mostarTablaEquipaje()
        {
         ...
         while($i<$rows2){
             if($nombre_tabla[$i]=='ModificarEquipaje'){
                 $j=1;
             }
             $i+=1;
         }
         ...
         if($j == 1){
            ...
            while ($data = mysqli_fetch_assoc($query))
            {
                ...
                "
                    <tr>
                        ...
                        <form action='formEditNave.php?id=$data[Id_Equipaje]' method='POST' name='form2'>
                        <th scope='row'>$data[Id_Equipaje]</th>
                        <td>$data[Peso]</td>
                        <td>$data[Cantidad]</td>
                        <td>$data[MultiplicadorPrecio]</td>
            ...
            "
                ...
                <table>
                <thead>
                <th scope='col'><a class='btn btn-danger' href='../Procesos/Reportes/equipajepdf.php'><i class='fa-solid fa-file-pdf'></i> Generar PDF
                </a></th>
                            <th scope='col'>
                            <a class='btn btn-success' href='../Procesos/Reportes/equipajexlsx.php'><i class='fa-regular fa-file-excel'></i> Generar EXCEL
                            </a></th>
                            </thead>
                            </table>
