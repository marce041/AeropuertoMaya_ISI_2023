<?php
    ...
    class elementosMenu
    {
        public function mostarTablaCiudad()
        {
         ...
         while($i<$rows2){
             if($nombre_tabla[$i]=='ModificarCiudad'){
                 $j=1;
             }
             $i+=1;
         }
         ...
         if($j == 1){
            ...
            while ($data = mysqli_fetch_assoc($query))
            {
                ...
                "
                    <tr>
                        ...
                        <form action='formEditNave.php?id=$data[Id_Ciudad]' method='POST' name='form2'>
                        <th scope='row'>$data[Id_Ciudad]</th>
                        <td>$data[Nombre]</td>
                        <td>$data[Codigo]</td>
                        <td>$data[Terminal]</td>
            ...
            "
                ...
                <table>
                <thead>
                <th scope='col'><a class='btn btn-danger' href='../Procesos/Reportes/ciudadpdf.php'><i class='fa-solid fa-file-pdf'></i> Generar PDF
                </a></th>
                            <th scope='col'>
                            <a class='btn btn-success' href='../Procesos/Reportes/ciudadxlsx.php'><i class='fa-regular fa-file-excel'></i> Generar EXCEL
                            </a></th>
                            </thead>
                            </table>
