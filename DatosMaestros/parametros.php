    ...
    <div id="wrapper">
        ...
        <div id="content" style="width:150%;">
                <nav class="navbar navbar-expand topbar mb-4 text-light shadow" style="background-color:#060724;text-color:white;">
                    ...
                    <h2 style="font-family:Luminari;text-color:white;"><i class="fa-solid fa-signal"></i> Parámetros de factura</h2>
                ...
                <div class="col-auto">
                    ...
                    <div class="card shadow mb-4">
                        ...
                        <div class="card-body">
                            <form class="row g-3 needs-validation" action="../Procesos/Guardar/parametroAdd.php"  method="POST" enctype="multipart/form-data">
                                <div class="mb-3">
                                  <input class="form-control" name="cai" type="text" pattern="[0-9A-Z][0-9A-Z][0-9A-Z][0-9A-Z][0-9A-Z][0-9A-Z][-][0-9A-Z][0-9A-Z][0-9A-Z][0-9A-Z][0-9A-Z][0-9A-Z][-][0-9A-Z][0-9A-Z][0-9A-Z][0-9A-Z][0-9A-Z][0-9A-Z][-][0-9A-Z][0-9A-Z][0-9A-Z][0-9A-Z][0-9A-Z][0-9A-Z][-][0-9A-Z][0-9A-Z][0-9A-Z][0-9A-Z][0-9A-Z][0-9A-Z][-][0-9A-Z][0-9A-Z]" title="El formato es: EJEMPLO 'CE5B57-D7BA6E-DF468C-8D8FE2-BF6E7D-4E'"  placeholder="CAI" onkeypress="return event.charCode>=48 && event.charCode<=57 || event.charCode>=65 && event.charCode<=90 ||event.charCode===45"  minlength="37" maxlength="37" required>
                                </div>
                                <script src="https://ajax.googleapis.com/ajax/libs/jquery/2.1.1/jquery.min.js"></script>
                                <div class="mb-3">
                                  <input class="form-control" name="fecha_emi" id="fecha_emi" type="date"  onclick="setminfecha();"  placeholder="Fecha de Emisión"  required >
                                </div>
                                <script>
                                    ...
                                    $("#fecha_emi").prop('min', formatDate(date));

                                    $( "#fecha_emi").change(function() {
                                    
                                    var date2 = $("#fecha_emi").val();
                                    //var value = $("#s").val();
                                    
                                    $("#fecha_ven").attr('min', date2);
                                    });
                                    
                                    
                               // document.getElementById('fecha_emi').min=today;    
                                }
                                </script>
                                <div class="mb-3">
                                  <input class="form-control" name="fecha_ven" id="fecha_ven" type="date" min="" placeholder="Fecha de Vencimiento" required>
                                </div>
                                <div class="mb-3">
                                  <input class="form-control" onchange="dividirCadena(this.value, '-');" pattern="[0][0][1][-][0][0][1][-][0][1][-][0-9][0-9][0-9][0-9][0-9][0-9][0-9][0-9]" title="El formato es 001-001-01-00000000"  name="rango_ini" type="text" placeholder="Rango Inicial" onkeypress="return event.charCode>=48 && event.charCode<=57 || event.charCode===45" required minlength="19" maxlength="19">
                                </div>
                                <script>

                                   function dividirCadena(cadenaADividir,separador) {
                                    var arrayDeCadenas = cadenaADividir.split(separador);
                                    var conse = Number(arrayDeCadenas[3])-1;
                                    arrayDeCadenas[3]=Number(arrayDeCadenas[3])+10;
                                    var cadena3= zfill(arrayDeCadenas[3],8);
                                    
                                    var separ= "-";
                                    var stringfin=arrayDeCadenas[0]+separ+arrayDeCadenas[1]+separ+arrayDeCadenas[2]+separ+cadena3; 
                                    $("#rango_fin").prop('value', stringfin);
                                    $("#consecutivo").prop('value',conse);
                                    $("#rangfin").prop('value', stringfin);
                                    $("#conse").prop('value',conse);
                                    } 
    ...
    if (width <= length) {
        ...
        if (number < 0) {
            ...
            return ((zero.repeat(width - length)) + numberOutput.toString()); 
                                ...
                                <div class="mb-3">
                                  <input class="form-control" name="rango_fin" id="rango_fin" type="text" pattern="[0][0][1][-][0][0][1][-][0][1][-][0-9][0-9][0-9][0-9][0-9][0-9][0-9][0-9]" title="El formato es 001-001-01-00000000"  name="rango_ini" type="text" readonly placeholder="Rango Final" onkeypress="return event.charCode>=48 && event.charCode<=57 || event.charCode===45" minlength="19" maxlength="19" required>
                                </div>
                                <div class="mb-3">
                                  <input class="form-control" name="consecutivo" id="consecutivo" readonly type="text" placeholder="Consecutivo" required >
                                </div>
                              ...
                              <div class="mb-3">
                              <input name="rangfin" id="rangfin" type=hidden>  
                            </div>
                            <div class="mb-3">
                              <input name="conse" id="conse" type=hidden>  
                            </div>
                      </form>
                        </div>
                    </div>
